...
namespace DamageNumbersPro
{
    public abstract class DamageNumber : MonoBehaviour
    {
        ...
        //Combination:
        public DamageNumber myAbsorber;
        bool givenNumber;
        ...
        Vector3 ApplyShake(Vector3 vector, ShakeSettings shakeSettings, float time)
        {
            ...
            {
                vector += DNPUpdater.upVector * currentFactor * shakeSettings.offset.y;
            }

            if (shakeSettings.offset.x != 0)
            {
                vector += DNPUpdater.rightVector * currentFactor * shakeSettings.offset.x;
            }
        ...
        void HandleDestruction(float time)
        {
            if (enableDestruction && isDestroyed)
            {
                ...
                {
                    baseAlpha = 0.9f * destructionSettings.alphaCurve.Evaluate(destructionProgress);
                    UpdateAlpha(currentFade);
